...
from src.utils.ties_layer_utils import ties_merging_layerwise
...
from src.utils.TSVM_utils import compute_and_sum_svd_mem_reduction, compute_and_sum_svd_mem_reduction_rsvd
from src.utils.iso import iso_c, iso_cts, isoc_ns_merge
from src.utils.pact_utils import pact_isoc_merge, pact_ta_merge, pact_isocts_merge, pact_tsvm_merge, pact_isoc_randsvd_merge, pact_ta_randsvd_merge, pact_tsvm_randsvd_merge
from src.utils.sift_utils import sift_ta_merge, sift_isoc_merge
...
def create_task_vector(
    ...
    if config.method.name not in ["TSVM", "TSVM_rsvd", "iso_c", "iso_cts", "isoc_ns", "ties-layer", "pact_isoc", "pact_ta", "pact_isocts", "pact_tsvm", "pact_tsvm_rsvd", "pact_isoc_rsvd", "pact_ta_rsvd", "sift_ta", "sift_isoc"]:
    ...
    if config.method.name in ["TSVM", "TSVM_rsvd", "iso_c", "iso_cts", "isoc_ns", "ties-layer", "pact_isoc", "pact_ta", "pact_isocts", "pact_tsvm", "pact_tsvm_rsvd", "pact_isoc_rsvd", "pact_ta_rsvd", "sift_ta", "sift_isoc"]:
    ...
    if config.method.name == "ties":
    ...
    elif config.method.name == "ties-layer":
        # TIES Merging Layer-wise
        merge_func = "dis-mean"

        # Prepare task vectors in dictionary format (layer-wise)
        task_vectors_dict = {}
        for key in ptm_check.keys():
            if key in remove_keys:
                continue
            # Stack task vectors for this parameter across all tasks
            task_vectors_list = []
            for check in ft_checks:  # ft_checks is a list
                task_vector = check[key] - ptm_check[key]
                task_vectors_list.append(task_vector)

            # Stack into (num_tasks, *param_shape)
            task_vectors_dict[key] = torch.stack(task_vectors_list, dim=0)

        # Apply layer-wise TIES merging
        merged_tv_dict = ties_merging_layerwise(
            task_vectors_dict, reset_thresh=config.method.k, merge_func=merge_func
        )

        # Convert back to flat vector format for compatibility
        ...
        print(f"=== Iso-C ===")
        use_magnitude_weighting = getattr(config.method, 'use_magnitude_weighting', False)
        new_merged_tv = iso_c(task_vectors, config,
                              use_magnitude_weighting=use_magnitude_weighting)
    elif config.method.name == "isoc_ns":
        print(f"=== Iso-C with Newton-Schulz ===")
        new_merged_tv = isoc_ns_merge(task_vectors, config)
    elif config.method.name == "TSVM":
        print(f"=== Using TSVM ===")
        new_merged_tv = compute_and_sum_svd_mem_reduction(task_vectors, config)
    elif config.method.name == "TSVM_rsvd":
        print(f"=== Using TSVM-RSVD (Randomized SVD) ===")
        new_merged_tv = compute_and_sum_svd_mem_reduction_rsvd(task_vectors, config)
    ...
    if config.method.name in ["TSVM", "TSVM_rsvd", "iso_c", "iso_cts", "isoc_ns", "pact_isoc", "pact_ta", "pact_isocts", "pact_tsvm", "pact_tsvm_rsvd", "pact_isoc_rsvd", "pact_ta_rsvd", "sift_ta", "sift_isoc"]:
