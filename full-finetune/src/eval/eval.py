...
def evaluate_task_vector(
    ...
    elif args.method.name in ["iso_c", "iso_cts", "TSVM", "TSVM_rsvd"]:
        scaling_coef_range = np.linspace(0.0, 5.0, args.n_eval_points)[1:]
    elif args.method.name in ["pact_ta", "pact_ta_rsvd"]:
        scaling_coef_range = np.linspace(2.5, 7.5, args.n_eval_points)[1:]
    elif args.method.name in ["pact_isoc", "pact_isoc_rsvd"]:
        scaling_coef_range = np.linspace(1.0, 6.0, args.n_eval_points)[1:]
    elif args.method.name in ["pact_isocts", "pact_tsvm", "pact_tsvm_rsvd"]:
        scaling_coef_range = np.linspace(1.0, 6.0, args.n_eval_points)[1:]
    elif args.method.name == "sift_ta":
        scaling_coef_range = np.linspace(3.0, 8.0, args.n_eval_points)[1:]
    elif args.method.name == "sift_isoc":
        scaling_coef_range = np.linspace(2.0, 7.0, args.n_eval_points)[1:]
    else:
