...
def pact_isoc_merge(lora_task_vectors, pretrained_vector, config):
    """
    PACT-IsoC 融合方法 (LoRA 版本)

    核心优势：
    1. LoRA 权重的更新量天然具有极低的秩 (Δ_t = B_t @ A_t)
    2. 直接对小矩阵 A_t 做 SVD 即可提取任务显式特征基底 V_t
    3. 融合后的矩阵重建为全尺寸的密集矩阵执行 Iso-C
    ...
    print(f"PACT-IsoC (LoRA) merging with {num_tasks} tasks")
    ...
    with torch.no_grad():
        ...
        print("Phase 3: Executing Iso-C merging...")
        new_vector = {}
        ...
        for base_name in all_base_names:
            ...
            if not is_lora:
                new_vector[base_name] = sum(deltas) / len(deltas)
                del deltas
            else:
                ...
                new_vector[base_name] = torch.linalg.multi_dot((U, torch.diag(S_mean), V))
                del U, S_mean, V
...
class PactIsoMerger:
    """
    PACT-IsoC Merger for LoRA-finetuned models.

    This merger implements the PACT (Pretrained Core Avoidance) method
    combined with Iso-C (Isotropic Merging) for LoRA models.
    ...
    4. Reconstruct full-size matrices for Iso-C merging
    """
    ...
    def transform(self, merge_config=None):
        """
        Transform method (required by the framework, but not used for PACT-IsoC).
        """
        return
    ...
    def merge(self, merge_config=None):
        ...
        merged_sd = pact_isoc_merge(lora_task_vectors, pretrained_weights, config)
